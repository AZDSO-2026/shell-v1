...
echo -e "\e[31m >>>>>>>>>>>>> Install App Dependencies & Build Html Code <<<<<<<<<<<<<<< \e[0m" | tee -a /tmp/roboshop.log
npm cache clean --force &>>/tmp/roboshop.log
npm install &>>/tmp/roboshop.log
npm run build &>>/tmp/roboshop.log

echo -e "\e[31m >>>>>>>>>>>>> Copy Built Code to Nginx <<<<<<<<<<<<<<< \e[0m" | tee -a /tmp/roboshop.log
rm -rf /usr/share/nginx/html/* &>>/tmp/roboshop.log
cp -r out/* /usr/share/nginx/html/ &>>/tmp/roboshop.log

echo -e "\e[31m >>>>>>>>>>>>> Start Nginx Service <<<<<<<<<<<<<<< \e[0m" | tee -a /tmp/roboshop.log
